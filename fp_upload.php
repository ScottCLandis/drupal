...
require_once dirname(__FILE__).'/adobe_helper.inc';
...
if (!isset($_SESSION['AdobeID']) || !isset($_SESSION['Password'])) {
	...
	if ($landscape) {
		...
		$fp->upload_cover($folioID, 'landscape', $landscape_temp_url);
	...
	if ($portrait) {
		...
		$fp->upload_cover($folioID, 'portrait', $portrait_temp_url);
		unlink($portrait_temp_url);
	}
	// attempts to upload the HTML Resources zip file
	$response = $fp->upload_htmlresources($folioID, 'styles/'.$style.'/HTMLResources.zip');
	...
	if ($folioID && $filenames) {
		...
		for ($i = 0; $i < sizeof($filenames); $i++) {
			...
			if ($filenames[$i] == '') { // checks if the article is not from Drupal
				...
				$fp->update_article($folioID, $alienated[$alienatedArrayCounter], array('sortOrder' => intval($adjustedSortOrder)));
				$alienatedArrayCounter++;
				continue;
			}
			$sourcePath	= 'folio/'.helper_format_title($filenames[$i]).'.folio';
			$response = $fp->upload_article($folioID, array('sortOrder' => intval($adjustedSortOrder)), $sourcePath);
			// locking the article
			//$fp->update_article($folioID, $response['articleInfo']['id'], array('locked' => 'true'));
			if ($response['status'] === 'ok') {
